function EvalStatisticsOnDBSCANandPoCResults(ClusterSmoothTableCh, Ch, outputFolder)

% Tabulate stats and output excel file for Clus-PoC analysis
...
for i=1:column
    ...
    for j=1:row
        ...
        if ~isempty(A)
           ...
           AA=cellfun(@(x) x(x.Nb > handles.PoC.NbThresh), A,'UniformOUtput',0);
           A=A(~cellfun('isempty', AA));
           
           % Cluster with Nb(Dof>0.4) > NbThresh      
           Cluster_DofC=cellfun(@(x) x(x.Nb_In > handles.PoC.NbThresh), A,'UniformOUtput',0);
           ...
           % Cluster with Nb(Dof>0.4) < NbThresh
           Cluster_Other=cellfun(@(x) x(x.Nb_In <= handles.PoC.NbThresh), A,'UniformOUtput',0);
...
%     Circularity3=cell2mat(MeanCircularity3(:));
    ...
    RegionName = strcat('Clus-PoC results');
    ...
    if ispc
      ...
      disp('WARNING: ignore XLS write in EvalStatisticsOnDBSCANandPoCResults');
    end
