function ResultTable = ProcessPoCResults(CellData, NDatacolumns, ROICoordinates, DensityROI, Path_name, ColoThres, settings)
...
mkdir(fullfile(Path_name, f1, 'Density and PoC maps')); % Plot 5
...
%                 disp(reg)
                ...
                Data_PoC1 = DataRaw(DataRaw(:, NDatacolumns + 7) == 1, :); % Data in this ROI in this Cell with Lr_r value above threshold
%%      
                if ~isempty(Data_PoC1)

                GenericName=strcat('Table_',num2str(cell),'_Region_',num2str(reg)); %t    
                % test Not a number value in column 5 6 and 12 (x y channel)
                
%                 TF=ismissing(Data_PoC1(:,:));
%                 [row, col]=find(TF==1);
%                 Data_PoC1(row,:)=[];
...
%                 DataNoOutliers(col,:)=[];
                ...
                x1 = Data_PoC1(Data_PoC1(:,12) == 1, 5); %  data from channel 1
                y1 = Data_PoC1(Data_PoC1(:,12) == 1, 6); %  data from channel 2
                x2 = Data_PoC1(Data_PoC1(:,12) == 2, 5);
                y2 = Data_PoC1(Data_PoC1(:,12) == 2, 6);

%                 D1_D2 = Data_PoC1.D1_D2; % Local Absolute Density per Channel
...
%                 Data_CoC1.NormDensity = Data_PoC1.Density./max(Data_PoC1.Density);
%                 D1_D2_Norm = Data_PoC1.D1_D2/max(Data_PoC1.D1_D2);
                %zn=Colo./max(Colo);
...
%                 end
                ...
                % Channel Ch1
                D1 = Data_PoC1(Data_PoC1(:,12) == 1, NDatacolumns + 6);
                PoC1 = Data_PoC1(Data_PoC1(:,12) == 1, NDatacolumns + 10);
                D1_Norm = D1/max(D1);
                
                Result.Correlation_Coloc1_vs_Density1 = corr(D1, PoC1, 'Type', 'Spearman');
                
                % Channel Ch2                
                D2 = Data_PoC1(Data_PoC1(:,12) == 2, NDatacolumns + 6);
                PoC2 = Data_PoC1(Data_PoC1(:,12) == 2, NDatacolumns + 10);
                D2_Norm = D2/max(D2);
                
                Result.Correlation_Coloc2_vs_Density2 = corr(D2, PoC2, 'Type', 'Spearman');
...
%%                                                
                ...
                % Colocalisation and Density map per C   
                    ...
                    tt = getframe(gcf);
                    imwrite(tt.cdata, fullfile(Path_name, f1, 'Density and PoC maps', sprintf('%sDensity_Ch%d.tif', GenericName, settings.TCR)));
                    if(settings.AlsoSaveFig)
                        saveas(gcf, fullfile(Path_name, f1, 'Density and PoC maps', sprintf('%sDensity_Ch%d.fig', GenericName, settings.TCR)));
                    end
                    ...
                    tt = getframe(gcf);
                    imwrite(tt.cdata, fullfile(Path_name, f1, 'Density and PoC maps', sprintf('%sDensity_Ch%d.tif', GenericName, settings.Signal)));
                    if(settings.AlsoSaveFig)
                        saveas(gcf, fullfile(Path_name, f1, 'Density and PoC maps', sprintf('%sDensity_Ch%d.fig', GenericName, settings.Signal)));
                    end
                    %saveas(gcf,strcat('Plot6\',GenericName ,'Plot6',strcat('_Ch',num2str(i))), 'tif');
                    close gcf
                    
                    figure
                    scatter( x1, y1,2,PoC1); 
                    axis equal
                    axis tight 
                    xlim(Xlimit);
                    ylim(Ylimit);
                    ...
                    tt = getframe(gcf);
                    imwrite(tt.cdata, fullfile(Path_name, f1, 'Density and PoC maps', sprintf('%sDoC_Ch%d.tif', GenericName, settings.TCR)));
                    if(settings.AlsoSaveFig)
                        saveas(gcf, fullfile(Path_name, f1, 'Density and PoC maps', sprintf('%sDoC_Ch%d.fig', GenericName, settings.TCR)));
                    end
                    %saveas(gcf,strcat('Plot6\',GenericName ,'Plot6',strcat('_Ch',num2str(i))), 'tif');
                    close gcf
                    
                    figure
                    scatter( x1, y1,2,log(PoC1+1)); 
                    axis equal
                    axis tight 
                    xlim(Xlimit);
                    ylim(Ylimit);
                    ...
                    tt = getframe(gcf);
                    imwrite(tt.cdata, fullfile(Path_name, f1, 'Density and PoC maps', sprintf('%sDoC_Ch%d.tif', GenericName, settings.Signal)));
                    if(settings.AlsoSaveFig)
                        saveas(gcf, fullfile(Path_name, f1, 'Density and PoC maps', sprintf('%sDoC_Ch%d.tif', GenericName, settings.Signal)));
                    end
                    %saveas(gcf,strcat('Plot6\',GenericName ,'Plot6',strcat('_Ch',num2str(i))), 'tif');
                    close gcf
                    
                    figure
                    scatter( x2, y2,2,PoC2); 
                    axis equal
                    axis tight 
                    xlim(Xlimit);
                    ylim(Ylimit);
                    ...
                    tt = getframe(gcf);
                    imwrite(tt.cdata, fullfile(Path_name, f1, 'Density and PoC maps', sprintf('%sPoC_Ch%d.tif', GenericName, 2)));
                    if(settings.AlsoSaveFig)
                        saveas(gcf, fullfile(Path_name, f1, 'Density and PoC maps', sprintf('%sDoC_Ch%d.fig', GenericName, 2)));
                    end
                    %saveas(gcf,strcat('Plot6\',GenericName ,'Plot6',strcat('_Ch',num2str(i))), 'tif');
                    close gcf
                    
                    figure
                    scatter( x2, y2,2,log(PoC2+1)); 
                    axis equal
                    axis tight 
                    xlim(Xlimit);
                    ylim(Ylimit);
                    ...
                    tt = getframe(gcf);
                    imwrite(tt.cdata, fullfile(Path_name, f1, 'Density and PoC maps', sprintf('%sPoC_Ch%d_log.tif', GenericName, 2)));
                    if(settings.AlsoSaveFig)
                        saveas(gcf, fullfile(Path_name, f1, 'Density and PoC maps', sprintf('%sPoC_Ch%d_log.fig', GenericName, 2)));
                    end
                    %saveas(gcf,strcat('Plot6\',GenericName ,'Plot6',strcat('_Ch',num2str(i))), 'tif');
                    close gcf
                end
...
%                 
%                 assignin('base', 'Data_PoC1', Data_PoC1);
...
%                 whos ColoThres
                ...
                Result.Percent_Ch1_ColocAbove04 = sum((Data_PoC1(:,NDatacolumns+10) >= ColoThres) & (Data_PoC1(:,12) == 1)) / sum((Data_PoC1(:,12) == 1)); % 
                Result.Percent_Ch2_ColocAbove04 = sum((Data_PoC1(:,NDatacolumns+10) >= ColoThres) & (Data_PoC1(:,12) == 2)) / sum((Data_PoC1(:,12) == 2)); % ;  %
...
%                 Result.AvRela_DensityBelow04=mean(d(Colo<ColoThres)/DensityROI_temp(1));
               ...
               % Normalized density is normalized to max value in any point in ROI
                ...
                NormdDensity = Data_PoC1(:, 8)/max(Data_PoC1(:,8));
                
                Result.AvNorm_DensityAbove04 = mean(NormdDensity(Data_PoC1(:, NDatacolumns + 10) >= ColoThres));
                Result.AvRela_DensityAbove04 = mean(Data_PoC1((Data_PoC1(:,NDatacolumns + 10) >= ColoThres), NDatacolumns + 8)/DensityROI{reg,cell}(1));
                Result.AvNorm_DensityBelow04 = mean(NormdDensity(Data_PoC1(:, NDatacolumns + 10) < ColoThres));
                Result.AvRela_DensityBelow04 = mean(Data_PoC1((Data_PoC1(:,NDatacolumns + 10) < ColoThres), NDatacolumns + 8)/DensityROI{reg,cell}(1));

                Result.Correlation_ColocVsDensity = corr(Data_PoC1(:, NDatacolumns + 8), Data_PoC1(:, NDatacolumns + 10), 'Type', 'Spearman');
                
                % Lr Above threshold channel 1
                
%                 Lr1=Data_PoC.Lr(Data_PoC.Ch==1);
%                 Lr1Norm=Lr1./max(Lr1);
%                 Result.PercentLrAbove05_Ch1=length(find(Lr1Norm>0.5))/length(Lr1);
%                 
%                 % Lr Above threshold channel 2
%                 Lr2=Data_PoC.Lr(Data_PoC.Ch==2);
%                 Lr2Norm=Lr2./max(Lr1);
%                 Result.PercentLrAbove05_Ch2=length(find(Lr2Norm>0.5))/length(Lr2);
                ...
                % Channel 1
%                 D1 = Data_PoC1(Data_PoC1(:,12) == 1, NDatachannels + 6);
                Result.Av_Rela_Density_Ch1 = mean(D1 / DensityROI{reg,cell}(2));
                Result.AvNorm_Density_Ch1 = mean(D1_Norm);
                
                % Above threshold
                D1_above_04 = Data_PoC1((Data_PoC1(:,12) == 1) & (Data_PoC1(:,NDatacolumns + 10) >= ColoThres), NDatacolumns + 6);
                ...
                % Below Threshold
%                 D1_below_04 = Data_PoC1.D1_D2((Data_PoC1.Ch == 1) & (Data_PoC1.PoC < ColoThres));
                D1_below_04 = Data_PoC1((Data_PoC1(:,12) == 1) & (Data_PoC1(:,NDatacolumns + 10) < ColoThres), NDatacolumns + 6);
                ...
                % Channel 2
%                 D2 = Data_PoC1.D1_D2(Data_PoC1.Ch == 2);
                Result.Av_Rela_Density_Ch2 = mean(D2 / DensityROI{reg,cell}(3));
                Result.AvNorm_Density_Ch2 = mean(D2_Norm);
                
                % Above threshold
%                 D2_above_04 = Data_PoC1.D1_D2((Data_PoC1.Ch == 2) & (Data_PoC1.PoC >= ColoThres));
                D2_above_04 = Data_PoC1((Data_PoC1(:,12) == 2) & (Data_PoC1(:,NDatacolumns + 10) >= ColoThres), NDatacolumns + 6);
                ...
                % Below Threshold
%                 D2_below_04 = Data_PoC1.D1_D2((Data_PoC1.Ch == 2) & (Data_PoC1.PoC < ColoThres));
                D2_below_04 = Data_PoC1((Data_PoC1(:,12) == 2) & (Data_PoC1(:,NDatacolumns + 10) < ColoThres), NDatacolumns + 6);
...
%                     PercentLrAbove05_Ch2];
    ...
    RegionName1 = strcat('PoC Results'); % name of the sheet
    if ispc
        xlswrite(fullfile(Path_name, 'PoC Results'), Array1, RegionName1, 'A1'); %'Regiion' = name of the filename xcel shee, Array1 = data to put in the spreadsheet, 'A1' where to start
        xlswrite(fullfile(Path_name, 'PoC Results'), Matrix_Result1, RegionName1, 'A2');
    end
